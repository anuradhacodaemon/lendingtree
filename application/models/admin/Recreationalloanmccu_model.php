...
class Recreationalloanmccu_model extends CI_Model {
    ...
    public function get_user($id = 0, $limit, $start, $filterData, $sortData = "") {
        ...
        $this->db->from(RECREATIONAL_MMCU_LOAN . ' as user');
    ...
    public function checklead_approved_forDomain() {
        ...
        $this->db->select('count(rc_id) as numLead');
        ...
        $this->db->where('active_status', 1);
        $this->db->from(RECREATIONAL_MMCU_LOAN);
        $result = $this->db->get();
    ...
    public function get_userdetails($shopId = 0) {
        $this->db->select('shop.*');
        $this->db->from(RECREATIONAL_MMCU_LOAN . ' as shop');
        // $this->db->join(LOGIN . ' as address', 'address.id = shop.contact_owner_id', 'left');
        $this->db->where('shop.rc_id', $shopId);
        $result = $this->db->get();
    ...
    public function updateactiveStatus($lendid) {
        ...
        $this->db->where("rc_id", $lendid);
        $this->db->update(RECREATIONAL_MMCU_LOAN, $userData);
