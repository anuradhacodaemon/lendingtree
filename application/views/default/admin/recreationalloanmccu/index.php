...
<!-- iCheck -->
<?php
$lead_approved = $this->Recreationalloanmccu_model->checklead_approved_forDomain();
$lend_pending = $this->Recreationalloanmccu_model->checklead_pending_forDomain();
$visitor = $this->Recreationalloanmccu_model->checklead_denied_forDomain();
?>
...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                <h2> Recreational Vehicle Loan Listing</h2>
            ...
            <form id="filter_form" method="get" action="">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 form-group ">
                        ...
                        <select name="type" class="form-control">
                            ...
                            <option value="6"<?php if (isset($_REQUEST['type'])) if ($_REQUEST['type'] == 6) echo 'selected'; ?>>Recreational Vehicle Loan</option>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-primary pull-right"  target="_blank" href="<?php echo BASE_URL . 'admin/recreationalloanmccu/export' ?>">Export as CSV</a>
                    </div>
                </div>
            </form>
                            ...
                            if (count($userlisting) > 0) {
                                ...
                                foreach ($userlisting as $k => $v) {
                                    ...
                                    <tr class="even pointer">
                                        ...
                                        <td>
                                            <input type="checkbox" id="check-all" name="c[]" value="<?php echo $v['rc_id'] ?>" class="flat" >
                                        </td>
                                        ...
                                        <td><a href="javascript:void(0)" id="fc_edit" data-toggle="modal" data-target="#CalenderModalView" style="color:green" onclick="getStatus(<?php echo $v['rc_id'] ?>,<?php echo $v['status'] ?>)" title="Change Status"><?php if ($v['status'] == 1) { ?>  Approved<?php } ?></a>
                                            <?php if ($v['status'] == 2) { ?> <a href="javascript:void(0)" id="fc_edit" data-toggle="modal" data-target="#CalenderModalView" style="color:darkgoldenrod" onclick="getStatus(<?php echo $v['rc_id'] ?>,<?php echo $v['status'] ?>)" title="Change Status">Pending <?php } ?>
                                                ...
                                                <!-- <td > 
                                                    ...
                                                    <a href="<?php //echo BASE_URL . MASTERADMIN . '/recreationalloanmccu/details/' . $v['rc_id'] ?>" class="dark_grey " ><i class="fa fa-eye"></i> </a>
                                                </td>  -->
                                        <td class="last">
                                            <a href="<?php echo BASE_URL . MASTERADMIN . '/recreationalloanmccu/details/' . $v['rc_id'] ?>" class="dark_grey" title="View" ><i class="fa fa-eye"></i> </a>
                                            <a href="<?php echo BASE_URL . MASTERADMIN . '/recreationalloanmccu/pdfloan/' . $v['rc_id'] ?>" target="_blank" class="dark_grey" title="View" ><i class="fa fa-file-pdf-o"></i> </a>

                                            &nbsp;&nbsp;
                                            <?php if ($v['active_status'] == 1) { ?>
                                                <a href="javascript:" onclick="inactiveFranchise(<?php echo $v['rc_id'] ?>)" style="color:#000" title="Delete"><i class="fa fa-times fa-cog-red" aria-hidden="true"></i></a>
                                            <?php } else { ?>
                                                <a href="javascript:"  onclick="activeFranchise(<?php echo $v['rc_id'] ?>)" style="color:#000"><i class="fa fa-times-circle-o fa-cog-red" aria-hidden="true"></i></a>
...
<script>
    $("#cPrice").click(function () {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url() . 'admin/recreationalloanmccu/updatestatus' ?>",
            ...
            success: function (data)
            {
                ...
                location.href = "<?php echo base_url() . 'admin/recreationalloanmccu' ?>";
            }
        });
    });
    ...
    function submitForm()
    {
        ...
        $("#filter_form").attr('action', '<?php echo BASE_URL . MASTERADMIN . '/recreationalloanmccu?' ?>' + vars);
        $("#filter_form").submit();
    }
    ...
    function inactiveFranchise(franchiseid) {
        bootbox.confirm("Do you want to delete this lead?", function (result) {
            if (result)
            {
                $.ajax({
                    type: "GET",
                    url: "<?php echo base_url() . 'admin/recreationalloanmccu/delete_inactive/' ?>" + franchiseid,
                    success: function (data)
                    {
                        location.href = '<?php echo base_url() . 'admin/recreationalloanmccu/' ?>';
                    }
        ...
        bootbox.confirm("Do you want to delete selected lead?", function (result) {
            if (result)
            {
                $.ajax({
                    ...
                    url: "<?php echo base_url() . 'admin/recreationalloanmccu/deleteall' ?>",
                    success: function (data)
                    {
                        //alert(data);
                        location.href = '<?php echo base_url() . 'admin/recreationalloanmccu/' ?>';
