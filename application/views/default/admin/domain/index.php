...
<button class="btn btn-dark" type="button" onclick="location.href = '<?php echo base_url() . 'admin/domain/add'; ?>'">Add</button> <div class="row">
                    ...
                    <table class="table table-striped responsive-utilities jambo_table bulk_action">
                        <thead>
                            <tr class="headings">
                                ...
                                <th class="column-title"><a href="javascript:void(0)" <?php if ($sort_by == "domain.phone" && $sort_direction == "desc") { ?> onClick="sortList('domain.phone', 'asc')"<?php } ?> <?php if ($sort_by != "domain.phone") { ?> onClick="sortList('domain.phone', 'asc')"<?php } ?><?php if ($sort_by == "domain.phone" && $sort_direction == "asc") { ?> onClick="sortList('domain.phone', 'desc')"<?php } ?>>phone</a>&nbsp;<?php if ($sort_by == "domain.phone" && $sort_direction == "asc") { ?><i class="fa fa-arrow-up" aria-hidden="true"></i>
                                    <?php } if ($sort_by == "domain.phone" && $sort_direction == "desc") { ?> <i class="fa fa-arrow-down" aria-hidden="true"></i> <?php } ?></th>
                            ...
                            if (count($domain) > 0) {
                                ...
                                foreach ($domain as $k => $v) {
                                    ...
                                    <tr class="even pointer">
                                        ...
                                        <td class=" "><?php echo $v['domain'] ?></td>
                                        <td class=" "><?php echo $v['emails'] ?></td>
                                        <td class=" "><?php echo $v['phone'] ?></td>
                                        <td class=" "><?php echo date('d-m-Y', strtotime($v['created_date'])); ?></td>

                                        <td class=" last"><a href="<?php echo base_url() . 'admin/domain/edit/' . $v['domain_id']; ?>" style="color:#000"><i class="fa fa-pencil"></i></a>
                                            &nbsp;
                                            <?php if ($v['status'] == 1) { ?>
                                                <a href="javascript:" onclick="inactiveFranchise(<?php echo $v['domain_id'] ?>)" style="color:#000"><i class="fa fa-check-circle fa-cog-green" aria-hidden="true"></i></a>
                                            <?php } else { ?>
                                                <a href="javascript:"  onclick="activeFranchise(<?php echo $v['domain_id'] ?>)" style="color:#000"><i class="fa fa-times-circle-o fa-cog-red" aria-hidden="true"></i></a>

                                            <?php } ?>
                                            &nbsp;
                                            <a href="<?php echo base_url() . 'admin/domain/view/' . $v['domain_id']; ?>" style="color:#000"><i class="fa fa-eye"></i></a>
                                        </td>
                                    </tr>
                                    <?php
                                    $i++;
