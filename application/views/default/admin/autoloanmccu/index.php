...
<!-- iCheck -->
<?php
$lead_approved = $this->Autoloanmccu_model->checklead_approved_forDomain();
$lend_pending = $this->Autoloanmccu_model->checklead_pending_forDomain();
$visitor = $this->Autoloanmccu_model->checklead_denied_forDomain();
?>
            ...
            <form id="filter_form" method="get" action="">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-primary pull-right"  target="_blank" href="<?php echo BASE_URL . 'admin/autoloanmccu/export' ?>">Export as CSV</a>
                    </div>
                </div>
            </form>
...
<script>
    $("#cPrice").click(function () {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url() . 'admin/autoloanmccu/updatestatus' ?>",
            ...
            success: function (data)
            {
                ...
                location.href = "<?php echo base_url() . 'admin/autoloanmccu' ?>";
            }
        });
    });
    ...
    function submitForm()
    {
        ...
        $("#filter_form").attr('action', '<?php echo BASE_URL . MASTERADMIN . '/autoloanmccu?' ?>' + vars);
        $("#filter_form").submit();
    }
    ...
    function inactiveFranchise(franchiseid) {
        bootbox.confirm("Do you want to delete this lead?", function (result) {
            if (result)
            {
                $.ajax({
                    type: "GET",
                    url: "<?php echo base_url() . 'admin/autoloanmccu/delete_inactive/' ?>" + franchiseid,
                    success: function (data)
                    {
                        location.href = '<?php echo base_url() . 'admin/autoloanmccu/' ?>';
                    }
        ...
        bootbox.confirm("Do you want to delete selected lead?", function (result) {
            if (result)
            {
                $.ajax({
                    ...
                    url: "<?php echo base_url() . 'admin/autoloanmccu/deleteall' ?>",
                    success: function (data)
                    {
                        //alert(data);
                        location.href = '<?php echo base_url() . 'admin/autoloanmccu/' ?>';
