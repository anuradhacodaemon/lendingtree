...
class Admin_template extends CI_Controller {
     ...
     */
        ...
        parent::__construct();
        $this->load->model('admin/Emailtemplate');
    ...
    public function index() {
        if (!isset($this->session->userdata['userdata']['ud'])) {
            ...
            $config['base_url'] = base_url() . MASTERADMIN . '/template';

            $config['total_rows'] = $this->Emailtemplate->get_count_domain($filterData);
            //$config['per_page'] = RECORD_LIMIT;
            ...
            $data["domain"] = $this->Emailtemplate->get_domain('', $config["per_page"], $page, $filterData, $sortData);
            $this->template->view('admin/template/index', array_merge($data, $filterData));
        }
    }
    ...
    public function add() {
        ...
        if (!isset($this->session->userdata['userdata']['ud'])) { $this->load->view('admin/index', $data);
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                $this->template->view('admin/template/add', $data);
            }
        }
    }
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($this->form_validation->run('template/insert') == true) {
                    ...
                    if ($result == 0) {
                     ...
                     $this->session->set_flashdata('item', array('message' => '<font color=red>' . validation_errors() . '</font>', 'class' => 'success'));
                    redirect('admin/template/edit/' . $id, $data);
                }
              
            } else {
                $last = $this->uri->total_segments();
                $id = $this->uri->segment($last);
                $data["domain"] = $this->Emailtemplate->get_domain($id, '', '', '', '');
                //print_r($id);exit;
                $this->template->view('admin/template/edit', $data);
            }
        }
    }
    ...
    public function delete_inactive($Id=0) {
        ...
        $data["contractor"] = $this->emailtemplate->inactive_domain($Id);

        redirect('admin/domain', 'refresh');
    }

    public function delete_active($Id=0) {
        $data["contractor"] = $this->emailtemplate->active_domain($Id);
        redirect('admin/domain', 'refresh');
    }
