...
class AutoLoanMccu extends CI_Controller {
    ...
    public function details($userId = 0) {
        ...
        if (!isset($this->session->userdata['userdata']['ud'])) {
            ...
            $data['userDetails'] = $this->Autoloanmccu_model->get_userdetails($userId);
            /* Load the view using template */
            $this->template->view('admin/autoloanmccu/userdetails', $data);
        }
    }
    //
    public function pdfLoan($id = 0) 
    {
        $data['userDetails'] = $this->Autoloanmccu_model->get_userdetails($id);
        $name = $data['userDetails']['first_name'] . '_' . $data['userDetails']['lend_id'];
        $pdf = new PDF();
        $pdf->SetTitle('' . $_SERVER['HTTP_HOST'] . '');
        $pdf->AddPage();
        $tbl = $this->load->view('mccu_auto_view_fileloan_auto_admin', $data, TRUE);
    ...
    public function export() {
        ...
        $arr = array($arr);
        $data2 = $this->Autoloanmccu_model->get_userall();
        // die;
        $type = '';
        $years = '';
        $a = array();
    ...
    {
        $this->Autoloanmccu_model->updateactiveStatus($lend_id);
    }
    //
    public function deleteall()
    {
        ...
        {
             $this->Autoloanmccu_model->updateactiveStatus($v);
        }
    }
    //
    public function updatestatus()
    {
        $this->Autoloanmccu_model->updateStatus($this->input->post('lendId'),$this->input->post('status'));
    }
    //
