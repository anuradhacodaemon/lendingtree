...
class Personalloanmccu_model extends CI_Model {
    ...
    public function get_user($id = 0, $limit, $start, $filterData, $sortData = "") {
        ...
        $this->db->from(PERSONAL_MMCU_LOAN . ' as user');
    ...
    public function checklead_approved_forDomain() {
        ...
        $this->db->select('count(p_id) as numLead');
        ...
        $this->db->where('active_status', 1);
        $this->db->from(PERSONAL_MMCU_LOAN);
        $result = $this->db->get();
    ...
    public function get_userdetails($shopId = 0) {
        $this->db->select('shop.*');
        $this->db->from(PERSONAL_MMCU_LOAN . ' as shop');
    ...
    public function updateactiveStatus($lendid) {
        ...
        $this->db->where("p_id", $lendid);
        $this->db->update(PERSONAL_MMCU_LOAN, $userData);
