    ...
    public function assign_officer() {
        ...
        if (!isset($this->session->userdata['userdata']['ud'])) { 
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                $loanOficcerAssigned = $this->loanofficer_model->loanOficcerAssigned($id,2);
                if ($this->form_validation->run('admin/asign_loanofficer') == true) {
                    ...
                    redirect(base_url().'admin/admin_refinance/assign_officer/' . $id, $data);
                } else {
                ...
                $loanOfficerDetails = $this->loanofficer_model->getLoanOficcerDetails($id,2);
                $data['userDetails'][0] = $this->refinance->get_userdetails($id);
...
//                print_r($data["userDetails"]);die();
                $this->template->view('admin/refinance/assign_officer', $data);
